      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Mass Communications</h1>
              <p className="text-gray-400 text-sm">Send templated bulk messages to filtered referral cohorts</p>
            </div>
          </div>
          <button onClick={() => setShowModal(true)}
            className="flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-500 rounded-xl text-sm text-white font-medium transition-all shadow-lg shadow-teal-500/25">
            <Plus className="w-4 h-4" /> New Campaign
          </button>
        </div>

        {/* Merge Fields Info */}
        <div className="bg-gray-900/60 border border-white/10 rounded-xl p-4 mb-6">
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Available Merge Fields</p>
          <div className="flex flex-wrap gap-2">
            {['{PatientName}', '{SpecialistType}', '{Status}', '{SlaDeadline}'].map(f => (
              <code key={f} className="bg-gray-800 text-blue-300 text-xs px-2.5 py-1 rounded-lg border border-blue-500/20">{f}</code>
            ))}
          </div>
        </div>
                ...
                {campaigns.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-white/2 transition-colors">
                      ...
                      <td className="px-5 py-4"><span className={`text-sm font-medium ${statusColor(c.status)}`}>{c.status}</span></td>
                      <td className="px-5 py-4 text-gray-400 text-sm">
                        <span className="text-emerald-400">{c.sentMessages}</span>
                        <span className="text-gray-600"> / {c.totalMessages}</span>
                        {c.failedMessages > 0 && <span className="text-red-400 ml-1">({c.failedMessages} failed)</span>}
                      </td>
                      ...
                      <td className="px-5 py-4 text-gray-400 text-sm">{c.createdByUser?.fullName ?? '—'}</td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-gray-400 uppercase tracking-wider mb-1.5 block">Campaign Name</label>
                <input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))}
                  placeholder="e.g. Urgent Cardiology Update"
                  className="w-full px-4 py-2.5 bg-gray-800 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50" />
              </div>
              <div>
                <label className="text-xs text-gray-400 uppercase tracking-wider mb-1.5 block">Subject Template</label>
              ...
              <div>
                <label className="text-xs text-gray-400 uppercase tracking-wider mb-1.5 block">Body Template</label>
                <textarea value={form.bodyTemplate} onChange={e => setForm(f => ({...f, bodyTemplate: e.target.value}))} rows={4}
                  ...
                  className="w-full px-4 py-2.5 bg-gray-800 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50 resize-none" />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-400 uppercase tracking-wider mb-1.5 block flex items-center gap-1.5"><Filter className="w-3 h-3" />Filter by Urgency</label>
                  <select value={form.urgencyFilter} onChange={e => setForm(f => ({...f, urgencyFilter: e.target.value}))}
                    className="w-full px-3 py-2.5 bg-gray-800 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50">
                    <option value="">All Urgencies</option>
                    {['Emergency','Urgent','Soon','Routine'].map(u => <option key={u}>{u}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-gray-400 uppercase tracking-wider mb-1.5 block flex items-center gap-1.5"><Filter className="w-3 h-3" />Filter by Status</label>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 border border-white/10 text-gray-400 rounded-xl text-sm hover:bg-white/5 transition-all">Cancel</button>
              <button onClick={handleSend} disabled={saving || !form.name || !form.bodyTemplate}
                className="flex-1 py-2.5 bg-teal-600 hover:bg-teal-500 text-white rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-all disabled:opacity-50">
