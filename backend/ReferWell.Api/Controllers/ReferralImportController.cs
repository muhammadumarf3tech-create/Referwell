...
public class ReferralImportController : ControllerBase
{
    ...
    public ReferralImportController(AppDbContext db) => _db = db;

    private Guid CurrentUserId =>
        Guid.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)
            ?? User.FindFirstValue(System.IdentityModel.Tokens.Jwt.JwtRegisteredClaimNames.Sub)!);

    // ── GET /api/referralimport ───────────────────────────────────────────────
    [HttpGet]
    ...
    public async Task<IActionResult> GetBatch(Guid id)
    {
        ...
        if (batch == null) return NotFound(new { message = "Import batch not found." });
    ...
    // ── GET /api/referralimport/{id}/rows ─────────────────────────────────────
        ...
        [FromQuery] int pageSize = 50)
    {
        ...
        var exists = await _db.ReferralImportBatches.AnyAsync(b => b.Id == id);
        if (!exists) return NotFound(new { message = "Import batch not found." });
    ...
    // ── GET /api/referralimport/template ──────────────────────────────────────
    [HttpGet("template")]
    public IActionResult DownloadTemplate()
    {
        ...
        return File(bytes, "text/csv", "referral-import-template.csv");
    }

    // ── POST /api/referralimport ──────────────────────────────────────────────
    [HttpPost]
    ...
    public async Task<IActionResult> Import(IFormFile? file)
    {
        ...
        try
        {
            (headers, dataRows) = ParseCsv(content);
        }
        catch (Exception ex)
        {
            return BadRequest(new { message = $"Unable to parse CSV: {ex.Message}" });
        }

        var missing = RequiredHeaders.Where(h => !headers.Contains(h, StringComparer.OrdinalIgnoreCase)).ToList();
        if (missing.Count > 0)
            return BadRequest(new { message = $"Missing required columns: {string.Join(", ", missing)}" });

        if (dataRows.Count == 0)
            return BadRequest(new { message = "The CSV has a header but no data rows." });

        if (dataRows.Count > 12_000)
            return BadRequest(new { message = "Maximum 12,000 rows per import. Split the file and try again." });
        ...
        try
        {
            var result = await ProcessImportAsync(batch, headers, dataRows);
        ...
        catch (Exception ex)
        {
            ...
            await _db.SaveChangesAsync();
            return StatusCode(500, new { message = "Import failed.", detail = ex.Message, batchId = batch.Id });
        }
    }
        ...
        List<Dictionary<string, string>> dataRows)
    {
        ...
        var nextCaseIndex = await GetNextCaseIndexAsync();
        var existingCaseNos = await _db.Referrals.AsNoTracking().Select(r => r.CaseNo).ToListAsync();
    ...
    private async Task<int> GetNextCaseIndexAsync()
    {
